...
def main():
    ...
    if request.method == "POST":
        ...
        if request.headers.get('X-Requested-With'):
            ...
            # test = jsonify(service)
            # response = redirect(url_for("form"))
            # response.set_cookie('auth_cook', test)
...
def form(credentials):
    ...
    if request.method == 'GET': #this block is only entered when the form is submitted
        ...
        # timeEst = int(request.args['est'])
        # DedLine = request.args['dead']

        print ('phase 1')
        print (title)
        print (timeEst)
        print (Dedline)
        return createEvent(title, timeEst, DedLine, credentials)
    else:
        # title = "title"
        # timeEst = 1
        # Dedline = "2018-11-21"
...
#
#     global event
#     eventTime = getEventTime(duration, deadLine, credentials)
#
#     # print(eventTime)
#
